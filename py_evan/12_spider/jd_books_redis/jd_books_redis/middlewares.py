...
class JdBooksRedisSpiderMiddleware:
...
class JdBooksRedisDownloaderMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        request.meta['proxy'] = "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": username, "pwd": password,
                                                                        "proxy": ':'.join(
                                                                            JdBooksRedisDownloaderMiddleware._proxy)}
    ...
    def process_exception(self, request, exception, spider):
        ...
        if "'status': 407" in exception.__str__():  # 不同版本的exception的写法可能不一样，可以debug出当前版本的exception再修改条件
            ...
            dnscache.__delitem__(JdBooksRedisDownloaderMiddleware._proxy[0])  # 删除proxy host的dns缓存
        return exception
